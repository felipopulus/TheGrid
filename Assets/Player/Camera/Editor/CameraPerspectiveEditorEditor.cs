...
public class CameraPerspectiveEditorEditor : Editor
{
	...
	void OnSceneGUI ()
	{
		...
		{
			Handles.color = new Color(1, 0, 0, 0.65f);

			viewMatrix = thisCamera.projectionMatrix;
			if (thisCamera.orthographic)
			{
				viewMatrix.m00 = 1 / thisCamera.aspect;
				viewMatrix.m11 = 1;
				
				//Begin - Fix for representing orthographic zoom correctly
				viewMatrix.m02 = lensShift.vector2Value.x / thisCamera.aspect;
				viewMatrix.m12 = lensShift.vector2Value.y;
			...
			{
				tr.x = tr.y = br.x = tl.y = 1;
				bl.x = bl.y = tl.x = br.y = -1;
			}
			bl.z = tl.z = br.z = tr.z = 1;
			
			viewMatrix = thisCamera.cameraToWorldMatrix * viewMatrix.inverse;
			
			bl1 = viewMatrix.MultiplyPoint(-tr);
			tl1 = viewMatrix.MultiplyPoint(-br);
			br1 = viewMatrix.MultiplyPoint(-tl);
			tr1 = viewMatrix.MultiplyPoint(-bl);
			bl2 = viewMatrix.MultiplyPoint(bl);
			tl2 = viewMatrix.MultiplyPoint(tl);
			br2 = viewMatrix.MultiplyPoint(br);
			tr2 = viewMatrix.MultiplyPoint(tr);
